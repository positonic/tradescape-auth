...
export const setupsRouter = createTRPCRouter({
  ...
  createFromTranscription: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const setupsData = await getSetups(session.transcription, 'trade-setups', availablePairs);
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      console.log('🔍 [Setup Debug] Transcription session ID:', setup.transcriptionSessionId);
      console.log('🔍 [Setup Debug] Transcription session object:', setup.transcriptionSession);
      ...
      console.log('🔍 [Setup Debug] Setup createdAt.getTime():', setup.createdAt.getTime());
      console.log('🔍 [Setup Debug] Setup timestamp (BigInt):', setupCreatedAtTimestamp);
      console.log('🔍 [Setup Debug] Current time:', new Date());
      console.log('🔍 [Setup Debug] Date.now():', Date.now());
      console.log('🔍 [Setup Debug] Current timestamp (BigInt):', BigInt(Date.now()));
      ...
      console.log('🔍 [Setup Debug] About to query trades for pairId:', setup.pairId);
      ...
      if (trades.length === 0 && setup.pair?.symbol) {
        ...
        console.log('🔍 [Setup Debug] Looking for trades containing:', setup.pair.symbol);
        ...
        console.log(`🔍 [Setup Debug] Found ${trades.length} trades with startsWith matching`);
        ...
        if (trades.length === 0) {
          ...
          console.log(`🔍 [Setup Debug] Found ${trades.length} trades with contains matching`);
        }
      }
      ...
      console.log('🔍 [Setup Debug] Recent trades without date filter:', recentTrades.length);
      if (recentTrades.length > 0) {
        console.log('🔍 [Setup Debug] Sample recent trade:', {
          ...
          isAfterSetup: Number(recentTrades[0]?.time) > Number(setupCreatedAtTimestamp)
        });
      }
      ...
      console.log('🔍 [Setup Debug] Sample pairIds from trades:', pairIdSample.map(t => ({ pairId: t.pairId, pair: t.pair })));
      ...
      if (setup.transcriptionSession?.createdAt) {
        const sessionCreatedAtTimestamp = BigInt(setup.transcriptionSession.createdAt.getTime());
        console.log('🟡 [TRADES DEBUG] Transcription session created at:', setup.transcriptionSession.createdAt);
        console.log('🟡 [TRADES DEBUG] Session timestamp (BigInt):', sessionCreatedAtTimestamp);
        console.log('🟡 [TRADES DEBUG] Session timestamp (Date):', new Date(Number(sessionCreatedAtTimestamp)));
        ...
        console.log(`🔍 [Setup Debug] Found ${allPositionsForPair.length} total positions for pair ${setup.pair?.symbol} (no time filter)`);
        ...
        console.log('🔍 [Setup Debug] Available position pair symbols:', positionPairSymbols.map(p => p.pair));
        ...
        console.log(`🔍 [Setup Debug] Found ${allOrdersForPair.length} total orders for pair ${setup.pair?.symbol} (no time filter)`);
        ...
        console.log('🔍 [Setup Debug] Available order pair symbols:', orderPairSymbols.map(o => o.pair));
        ...
        console.log(`🟡 [TRADES DEBUG] Found ${allTradesForPair.length} total trades for pair ${setup.pair?.symbol} (no time filter)`);
        ...
        console.log(`🔍 [Setup Debug] Found ${additionalPositions.length} total positions since session`);
        console.log(`🔍 [Setup Debug] Found ${additionalOrders.length} total orders since session`);
        ...
        console.log(`🟡 [TRADES DEBUG] Found ${additionalTrades.length} total trades within 24 hours of session`);
        ...
        if (additionalTrades.length > 0) {
          ...
          additionalTrades.forEach((trade, index) => {
            console.log(`🟡 [TRADES DEBUG] Session Trade ${index + 1}:`, {
              ...
              price: trade.price,
              vol: trade.vol,
              isAfterSession: Number(trade.time) >= Number(sessionCreatedAtTimestamp)
            });
          });
        } else {
          console.log('🟡 [TRADES DEBUG] NO TRADES FOUND SINCE SESSION - checking time comparison...');
      ...
      const serializedAllTradesSinceSession = allTradesSinceSession.map(trade => ({
        ...trade,
        closedPnL: trade.closedPnL ? Number(trade.closedPnL) : null,
        price: trade.price ? Number(trade.price) : null,
        cost: trade.cost ? Number(trade.cost) : null,
      ...
      const serializedAllOrdersSinceSession = allOrdersSinceSession.map(order => ({
        ...order,
        amount: Number(order.amount),
        totalCost: Number(order.totalCost),
        fee: Number(order.fee),
      ...
      console.log("serializedAllOrdersSinceSession is ", serializedAllOrdersSinceSession);
      const serializedAllPositionsSinceSession = allPositionsSinceSession.map(position => ({
        ...position,
        averageEntryPrice: Number(position.averageEntryPrice),
        averageExitPrice: Number(position.averageExitPrice),
        totalCostBuy: Number(position.totalCostBuy),
      ...
      console.log("serializedAllPositionsSinceSession is ", serializedAllPositionsSinceSession);
      console.log("serializedAllTradesSinceSession is ", serializedAllTradesSinceSession);
      return {
        ...setup,
