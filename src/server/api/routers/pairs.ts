...
import {
  DefaultTradeMapper,
  DefaultOrderMapper,
} from "~/app/tradeSync/repositories/mappers/tradeMappers";
...
export const pairsRouter = createTRPCRouter({
  ...
  validatePositionCreation: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      console.log(
        `📊 Found ${positions.length} automated positions to analyze`,
      );
      ...
      for (const position of positions) {
        ...
        // Analyze order types
        const buyOrders = position.orders.filter(
          (order) => order.type === "buy",
        );
        const sellOrders = position.orders.filter(
        ...
        if (buyOrders.length > 0 && sellOrders.length > 0) {
          ...
          const balancePercentage =
            avgVolume > 0 ? (volumeDiff / avgVolume) * 100 : 0;
      ...
      const analysis = {
        ...
        sellOnlyPositions,
        positionsByPair,
        averageOrdersPerPosition:
          positions.length > 0 ? totalOrders / positions.length : 0,
        averageProfitLoss:
          positions.length > 0 ? totalProfitLoss / positions.length : 0,
        positivePositions,
        negativePositions,
        breakEvenPositions,
        completenessScore:
          positions.length > 0
            ? (completePositions / positions.length) * 100
            : 0,
        profitabilityScore:
          positions.length > 0
            ? (positivePositions / positions.length) * 100
            : 0,
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (dryRun) {
          ...
          const positionAggregator =
            EnhancedPositionAggregator.createForStrategy("positionByDirection");
        ...
        if (positions.length > 0) {
          ...
          console.log(
            `✅ Successfully saved ${savedPositions.length} positions`,
          );
  ...
  syncTrades: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const syncResult = await tradeSyncService.syncTrades(
          userId,
          encryptedKeys,
          mode,
          since,
        ...
        if (syncResult.success && syncResult.tradesFound > 0) {
          console.log(
            `💾 Saving ${syncResult.tradesFound} trades to database...`,
          );
          ...
          if (error || !userExchange) {
            console.error(
              "Failed to initialize userExchange for saving trades:",
              error,
            );
            return syncResult;
          }
          ...
          if (trades.allTrades.length > 0) {
            ...
            if (orders.length > 0) {
              ...
              console.log(
                `🏗️  Creating positions from ${savedOrders.length} orders...`,
              );
              try {
                const positionAggregator =
                  EnhancedPositionAggregator.createForStrategy(
                    "positionByDirection",
                  );
            ...
            console.log(
              `✅ Successfully saved ${trades.allTrades.length} trades and ${orders.length} orders`,
            );
          }
        }
  ...
  deleteAllPositions: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      const userId = ctx.session.user.id;
      ...
      console.log(
        `🔗 Found ${positions.length} positions with ${positions.reduce((sum, p) => sum + p.orders.length, 0)} linked orders`,
      );
