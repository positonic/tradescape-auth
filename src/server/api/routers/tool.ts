  ...
  chat: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
          ...
          response.tool_calls.map(async (toolCall) => {
            ...
            try {
              ...
              switch (toolCall.name) {
                case "video_search":
                  toolResult = (await createVideoSearchTool(ctx).invoke(
                    toolCall.args as VideoSearchArgs,
                  )) as ToolResponse;
                  break;
                case "add_video":
                  toolResult = (await createAddVideoTool(ctx).invoke(
                    toolCall.args as VideoArgs,
                  )) as ToolResponse;
                  break;
                // case "create_action":
                ...
                //     break;
                case "market_scan":
                  toolResult = (await traderTools.marketScanTool.invoke(
                    toolCall.args as TranscriptionArgs,
                  )) as ToolResponse;
                  break;
                ...
                //     break;
                case "gm":
                  toolResult = (await gmTool().invoke(
                    toolCall.args as VideoArgs,
                  )) as ToolResponse;
                  break;
                default:
              ...
              if (toolCall.name === "market_scan") {
                try {
                  ...
                  console.error(
                    "Failed to parse market scan result:",
                    parseError,
                  );
                }
              }
              ...
              messages.push(
                new ToolMessage(
                  String(toolResult.result ?? toolResult.text ?? ""),
                  toolCall.id ?? "",
                ),
              );
        ...
        const validToolResults: ToolCallResult[] = toolResults.filter(
          Boolean,
        ) as ToolCallResult[];
