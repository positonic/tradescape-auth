...
const alertTypeOptions = Object.values(AlertType).map((val) => ({
  value: val,
  label: val,
}));
const directionOptions = Object.values(Direction).map((val) => ({
  value: val,
  label: val,
}));
...
export function AlertModal({ opened, onClose }: AlertModalProps) {
  ...
  const { data: pairsData, isLoading: isLoadingPairs } =
    api.pairs.getAll.useQuery();
  ...
  const form = useForm<AlertFormValues>({
    ...
    validate: {
      ...
      interval: (value, values) =>
        values.type === AlertType.CANDLE && !value
          ? "Interval is required for CANDLE alerts"
          : null,
    },
  });
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Create New Alert" centered>
      <LoadingOverlay
        visible={isLoadingPairs || createAlertMutation.isPending}
      ...
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack gap="md">
          ...
          <Group justify="flex-end" mt="md">
            ...
            <Button type="submit" loading={createAlertMutation.isPending}>
              Create Alert
            </Button>
          </Group>
        </Stack>
