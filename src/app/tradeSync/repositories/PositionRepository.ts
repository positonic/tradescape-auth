...
export class PositionRepository {
  ...
  async saveAll(
    ...
    for (const position of positions) {
      try {
        ...
        console.log(`✅ [PositionRepository] Created position ${createdPosition.id} for ${position.pair}`);
      } catch (error) {
        console.error(`❌ [PositionRepository] Failed to save position for ${position.pair}:`, error);
      }
    }

    console.log(`📊 [PositionRepository] Successfully saved ${savedPositions.length} positions`);
  ...
  private async findOrCreatePair(pairSymbol: string): Promise<{ id: number } | null> {
    try {
      ...
      if (!pairRecord) {
        ...
        console.log(`📝 [PositionRepository] Created new Pair record for ${pairSymbol}`);
      }

      return pairRecord;
    } catch (error) {
      console.error(`❌ [PositionRepository] Error finding/creating pair ${pairSymbol}:`, error);
      return null;
    }
  }
  ...
  async deletePosition(positionId: number, userId: string): Promise<boolean> {
    try {
      ...
      console.error(`❌ [PositionRepository] Error deleting position ${positionId}:`, error);
      return false;
    }
  }
}