...
export default function Chat({
  ...
  const chat = api.tools.chat.useMutation({
    onSuccess: async (results) => {
      ...
      if (marketScanResult) {
        try {
          const tradeSetupsResult = JSON.parse(marketScanResult.result) as {
            coins: CoinData[];
          };
          const tradeSetups = tradeSetupsResult.coins?.map(
            (coin: CoinData) => ({
    ...
    Promise.all(
        ...
        coin.tradeSetups.map(async (setup: CoinSetup, setupIndex: number) => {
          if (selectedSetups.includes(setupIndex)) {
            try {
              ...
              return {
                content:
                  setup.transcriptExcerpt ??
                  `${setup.position} setup for ${coin.coinSymbol}`,
                entryPrice: extractNumber(setup.entryPrice),
                takeProfitPrice: setup.t1
                  ? parseFloat(setup.t1)
                  : extractNumber(setup.takeProfit),
              ...
              console.error(
                `Failed to get pair for ${coin.coinSymbol}:`,
                error,
              );
              notifications.show({
                title: "Error",
      ...
      .then((setupPromises) => {
        return Promise.all(
          setupPromises
            .filter(
              (setup): setup is NonNullable<typeof setup> => setup !== null,
            )
      ...
      <Stack h="100%">
                ...
                >
                  ...
                  <Paper
                    ...
                    style={{
                      maxWidth: "100%",
                      backgroundColor:
                        message.type === "human" ? "#228be6" : "#f1f3f5",
                    }}
                  >
                    <Text
                      size="sm"
                  ...
                  {message.type === "human" && (
                    <Avatar size="md" radius="xl" src={null} alt="User" />
                  )}
                </Group>
                {message.tradeSetups && (
                  <Box mt="sm" style={{ width: "100%" }}>
                    ...
                    />
                    <br />
                    Select any setups you would like to save using the
                    checkboxes above.
                    <br />
                    <br />
                    <Link
        ...
        {selectedSetups.length > 0 && (
          <Button
            ...
            color="blue"
          >
            Save {selectedSetups.length} Selected Setup
            {selectedSetups.length > 1 ? "s" : ""}
          </Button>
        )}
