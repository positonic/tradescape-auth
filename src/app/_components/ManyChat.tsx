...
export default function ManyChat({
  ...
  const colors = {
    ...
    userMessageBg:
      colorScheme === "dark" ? theme.colors.blue[7] : theme.colors.blue[6],
  ...
  const parseAgentMention = (
    text: string,
  ): { agentId: string | null; cleanMessage: string } => {
    const mentionRegex = /@(\w+)/;
    const match = mentionRegex.exec(text);

    if (match && mastraAgents) {
      const mentionedName = match[1];
      const agent = mastraAgents.find(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { agentId: mentionedAgentId, cleanMessage } =
      parseAgentMention(input);
      ...
      return (
        <ReactMarkdown
          ...
          components={{
            ...
            ul: ({ children }) => (
              <Box component="ul" ml="md" mb="xs">
                {children}
              </Box>
            ),
            ol: ({ children }) => (
              <Box component="ol" ml="md" mb="xs">
                {children}
              </Box>
            ),
            li: ({ children }) => (
                ...
                {message.type === "ai" ? (
                  ...
                  >
                    ...
                    <Tooltip label="User" position="right" withArrow>
                      <Avatar size="md" radius="xl" alt="User">
                        {getInitials("User")}
                      </Avatar>
                    </Tooltip>
                  </Group>
            ...
            <div style={{ flex: 1, position: "relative" }}>
              ...
              {showAgentDropdown && (
                <Paper
                  ...
                  p="xs"
                >
                  {getFilteredAgentsForMention(input, cursorPosition).map(
                    (agent, index) => (
                      <div
                        key={agent.id}
                  ...
                  {getFilteredAgentsForMention(input, cursorPosition).length ===
                    0 && (
