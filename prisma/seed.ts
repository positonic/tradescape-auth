...
async function main() {
  ...
  for (const exchange of exchanges) {
    try {
      ...
      console.log(
        `✅ Created/Updated exchange ${result.name} with id: ${result.id}`,
      );
    } catch (error) {
      console.error(
        `❌ Failed to create/update exchange ${exchange.name}:`,
        error,
      );
    }
  }
  ...
  for (const pairSymbol of tradingPairs) {
    try {
      ...
      console.log(
        `✅ Created/Updated pair ${result.symbol} with id: ${result.id}`,
      );
    } catch (error) {
  ...
  console.log(
    "⚠️  Skipping UserPair creation - replace sampleUserId with actual user ID for testing",
  );
